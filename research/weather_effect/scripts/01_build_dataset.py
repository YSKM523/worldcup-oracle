...
# underdog 不败 = 爆冷(favorite 没赢): 结果不是 favorite 那一侧的胜
def is_upset(r):
    if pd.isna(r.fav_side):
        return np.nan
    fav_won = (r.outcome == r.fav_side)
    return 0 if fav_won else 1
df["upset"] = df.apply(is_upset, axis=1)
# favorite 是否取胜(用于时段/天气对 favorite 的影响)
df["fav_won"] = df.apply(lambda r: np.nan if pd.isna(r.fav_side) else int(r.outcome == r.fav_side), axis=1)
...
cols = ["espn_id","date","kickoff_utc","stage","home_team","away_team",
        "home_score","away_score","total_goals","gd","outcome",
