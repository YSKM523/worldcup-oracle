...
>
> **✅ 唯一被"证实"的效应在体能层（角度⑥，FIFA 官方数据）**：气温每升 1°C，一场比赛双方合计跑动**减少约 0.6 km**；从最凉到最热（16→33°C）合计少跑约 10 km。相关 **ρ=−0.31（p=0.009）**，用**体感温度**更强（ρ=−0.40, p=0.0007），**扣掉比赛节奏后依然显著（p=0.01）**，且**扛住了海拔混杂复核**——控制海拔后 r=−0.37、把墨西哥城/瓜达拉哈拉两座高原城市整个删掉后效应不降反升到 −0.38（本届高原恰好并不更热，混杂前提本就不成立）。不是"热场进球少所以跑得少"，也不是"高原压跑动"，是高温本身压低了耐力输出。**但冲刺距离不受影响（ρ=−0.10, p=0.41）**：爆发力保住、耐力下降，与运动生理学一致。
>
> **❌ 这份体能损耗并没有传导到结果上**：进球总数与温度零相关（角度②）；进球时间没有被推向下半场/终场（角度⑤，甚至凉爽场更"后置"）；爆冷/强队被拉平只有 p≈0.11 的微弱同向信号（角度①④），达不到显著。开球时段、湿度全程无信号。
>
...
**关键方法**：
- **爆冷 (upset)** 定义为"模型看好的一方没赢"（输或平），而非简单主客队胜负——这样才能把"谁强"从"天气"里剥离。
...
- 全样本爆冷率 **39.7%**（CI 29.6–50.8%）。
- 分桶：凉爽(<22°C) **33.3%**、温和(22–27°C) **32.1%**、**酷热(≥27°C) 51.7%**。酷热档明显偏高，但置信区间与其它档**重叠**（酷热 34–69%）。
- 相关：温度 vs 爆冷 ρ=0.144（p=0.22）；**扣掉实力后偏相关 r=0.179（p=0.12）**——扣实力后信号不降反略升，方向支持"热天利于弱队"，但没过显著性门槛。
- 湿度、热湿指数：无信号（ρ≈−0.08 / 0.00）。
...
| 傍晚 (18–21) | 38 | 2.66 | 24.0°C | **45.9%** |
| 夜间 (≥21) | 6 | 3.50 | 24.4°C | 33.3% |

- 爆冷最多的是**最凉爽的傍晚场**（45.9%），而非最热的正午场——这与"热→爆冷"的时段版本相矛盾。
- 进球随时段也无单调趋势。**时段本身不是一个干净的解释变量**（本届正午与傍晚温差仅约 3°C，多数场次集中在傍晚）。

### 角度④ 模型残差 vs 天气 —— **最连贯的一条线索**

- Brier（整体误差）vs 温度：ρ=0.045（p=0.70），无关。
- **校准残差**（favorite 实际胜率 − 模型预测胜率）vs 温度：r=**−0.182**（p=0.11）。分桶：凉爽 **+0.115**、温和 +0.074、**酷热 −0.093**。
- 解读：**模型看好的强队，在凉爽场里赢得比预期多、在酷热场里赢得比预期少**。这与角度①独立指向同一件事——**高温轻微地"拉平"强弱队**。这是全篇最值得继续追的信号，但仍 p≈0.11。
...
| 合计冲刺距离(Zone4) | ρ=−0.10 (p=0.41) ❌ | ρ=−0.19 (p=0.12) | — |
...
**三个要点：**
1. **高温显著压低耐力输出**——这是全研究唯一 p<0.01 的结论，且**扣掉"比赛是否开放/进球多少"后依然显著**，说明不是"热场节奏慢"的副产品，而是高温对身体的直接作用。**体感温度**（温度×湿度）比纯气温更强，符合生理学。
2. **冲刺（爆发力）不受高温影响**——Zone-4 冲刺距离与温度无关。耐力掉、爆发力保，与 Sky Sports 用 FIFA 数据得到的"最高冲刺速度不受高温影响"完全一致。
...
1. **样本太小**：79 场，分桶后每档仅 20–30 场；置换检验下 p≈0.11 的信号，扩到全部 104 场（决赛阶段结束）后才可能证实或证伪。
2. **实力是绝对主导**：天气的边际作用即便真实也很小，容易被球队强弱、临场状态淹没。
3. **共线性**：温度/体感/时段/城市彼此纠缠（如墨西哥城海拔高、迈阿密湿热），本研究**未能分离"高温"与"高海拔/特定城市"**——酷热档里若恰好挤了几支弱队的比赛，就会制造伪信号。
4. **体能数据仅限小组赛队级**：角度⑥的 FIFA 数据只有 72 场小组赛已发布、且只稳定解析到**队级**总跑动/冲刺（球员级数字在 PDF 里是图形层，未解析）；R32 起的 PMSR 尚未发布。
5. ~~海拔混杂尚未拆~~ ✅ **已复核（角度⑥稳健性）**：控制海拔/剔除高原城市后温度效应不降反升，非伪相关。剩余可控项是"城市固定效应检验力不足"（城内温差太小），需更多样本才能从城内变异独立确认。
6. **非因果**：以上全部为**探索性相关**，不构成因果，更不构成投注建议。

---

## 四、结论与后续建议

**给你的判断**：选题**成立**。高温对本届世界杯的影响是**真实的、但只在体能层显著**（跑动 ρ=−0.31, p<0.01），并没有变成记分牌上的 alpha——**不能作为 WorldCup Oracle 的预测因子上线**（爆冷/胜负那条线仍只 p≈0.11）。真正干净的产出是那句生理结论：**美洲酷暑让球员少跑、比赛变慢，但两队对称受累，胜负结构基本不动。**

**建议的下一步（按性价比排序）**：
1. ~~拆海拔混杂~~ ✅ 已完成（角度⑥稳健性，效应扛住并增强）。
2. **等样本 + 补 R32 起的 PMSR**（现最高优先）：决赛阶段打完后重跑，N 从 79→104、体能样本从 71→更多——既看爆冷那条 p≈0.11 会不会转正，也让城市固定效应（现因城内温差太小而检验力不足）有机会独立证实跑动效应。
...
*脚本*：`01_build_dataset.py`（天气对齐）、`02_analyze.py`（角度①–④）、`03_fetch_halftime.py`+`04_analyze_fatigue.py`（角度⑤半场）、`05_fetch_fifa_physical.py`+`06_analyze_physical.py`（角度⑥ FIFA 跑动）、`07_altitude_control.py`（角度⑥海拔稳健性）
